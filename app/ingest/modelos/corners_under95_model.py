...
def _score_row(row: pd.Series) -> Tuple[float, float]:
    ...
    over85 = _norm(row.get("corners_o85_potential"), 100.0)
    over95 = _norm(row.get("corners_o95_potential"), 100.0)
    under_frac = 1.0 - over95
    odds = row.get("Odds_U95") or row.get("odds_corners_under_95")
    odds = float(odds) if odds and odds > 1 else np.nan

    score = 2.1 + 2.6 * under_frac - 1.2 * total_pot - 1.0 * over85
    prob = _sigmoid(score)
